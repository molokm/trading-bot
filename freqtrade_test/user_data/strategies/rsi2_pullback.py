...
# flake8: noqa: F401
# isort: skip_file
"""Rsi2Pullback — RSI(2) откат в долгосрочном аптренде.

Покупка экстремально перепроданных коротких свечей (RSI(2) < порога) внутри
устойчивого аптренда (цена > 200-дневной SMA). Выход по RSI(2) > 65 или трейлинг.
Лонг-онли, в медвежий режим — кэш. Отличается высокой частотой сделок.
"""
...
class Rsi2Pullback(IStrategy):
    ...
    rsi2_enter = IntParameter(low=5, high=25, default=15, space="buy", optimize=True, load=True)
    rsi2_exit = IntParameter(low=55, high=80, default=65, space="sell", optimize=True, load=True)
    max_hold = IntParameter(low=2, high=10, default=4, space="sell", optimize=True, load=True)
    ...
    def custom_exit(
        ...
        if hold_days >= self.max_hold.value:
            return "time_stop"
        return None
