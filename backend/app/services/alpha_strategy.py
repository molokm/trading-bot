"""Alpha Strategy — Aggressive variant of Rotation v2.

Differences from base Rotation v2:
  - Risk 3% per trade (vs 2%)
  - Wider trailing: ATR x 0.8 (vs 0.5)
  - Partial TP at +7% / 40% (vs +5% / 50%)
  - Breakeven after 2% (vs 3%)
  - Longer min hold: 5 days (vs 3)
  - Higher ADX threshold: 22 (vs 18)
"""
...
STRATEGY_DESC = (
    "Alpha: aggressive rotation v2, risk 3%/trade, wider trailing (ATRx0.8), "
    "partial TP +7%/40%, breakeven 2%, ADX>22, min hold 5d."
)
...
class AlphaConfig:
    ...
    risk_per_trade: float = 0.03       # 3% risk per trade (aggressive)
    trail_atr_mult: float = 0.8       # wider trailing = ATR x 0.8
    breakeven_pct: float = 0.02       # move to BE after 2%
    partial_tp_pct: float = 0.07      # close 40% at +7%
    partial_tp_ratio: float = 0.4     # fraction to close
    rsi_period: int = 14
...
@dataclass
class AlphaPosition:
    """Open position in the alpha strategy."""
    symbol: str
    coin: str
    inst_id: str
...
class AlphaStrategy:
    def __init__(self, config: AlphaConfig, client_manager=None, db=None):
        self.config = config  # type: AlphaConfig
    ...
    async def _fetch_candles(self, client, coin: str, bar: str = "1D", limit: int = 100) -> list:
        ...
        if resp.get("error"):
            print(f"[Alpha] {coin} {bar} candles error: {resp.get('message', '')}", flush=True)
            return []
        data = resp.get("data", [])
        candles = []
        for c in data:
    ...
    def _check_correlation(self, candidate_coin: str, all_indicators: dict) -> bool:
        ...
        for held_coin in self._positions:
            ...
            if abs(corr) > self.config.corr_threshold:
                print(f"[Alpha] Correlation filter: {candidate_coin} corr with {held_coin} = {corr:.2f} > {self.config.corr_threshold} -> SKIP",
                      flush=True)
                return False
        return True
    ...
    async def _close_partial(self, client, inst_id: str, pos: AlphaPosition, close_ratio: float) -> dict:
        ...
        resp = await self._place_order(client, inst_id, close_side, close_sz,
                                          pos_side=pos.side)
        if resp.get("error"):
            print(f"[Alpha] Partial close error {pos.coin}: {resp.get('message', '')}", flush=True)
            return {}
        fills = resp.get("data", [])
        ...
        print(f"[Alpha] PARTIAL {now[:19]} {pos.coin:4} {pos.side:5} "
    ...
    async def _close_position(self, client, inst_id: str, pos: AlphaPosition, reason: str):
        ...
        resp = await self._place_order(client, inst_id, close_side, pos.size,
                                          pos_side=pos.side)
        if resp.get("error"):
            print(f"[Alpha] Close error {pos.coin}: {resp.get('message', '')}", flush=True)
            return
        ...
        if self.db:
            try:
                await self.db.save_trade(
                    bot_id=ALPHA_BOT_ID, side=close_side, sz=pos.size,
                    px=round(fill_px, 2),
                ...
                print(f"[Alpha] DB save trade error: {e}", flush=True)

        print(f"[Alpha] CLOSE  {now[:19]} {pos.coin:4} {pos.side:5} "
    ...
    async def _open_position(self, client, coin: str, side: str, ind: dict,
        ...
        if lev != 1.0:
            ...
            if lev_resp.get("error"):
                print(f"[Alpha] Set leverage error {coin}: {lev_resp.get('message', '')}", flush=True)
        ...
        if self.db:
            try:
                ...
                print(f"[Alpha] DB save signal error: {e}", flush=True)

        if not self.config.auto_execute:
            print(f"[Alpha] SIGNAL (no execute) {coin} {side} @ {price:.1f} lev={lev}", flush=True)
            return
        ...
        if resp.get("error"):
            print(f"[Alpha] Open error {coin}: {resp.get('message', '')}", flush=True)
        ...
        if self.db:
            try:
                ...
                await self.db.save_trade(
                    bot_id=ALPHA_BOT_ID, side=order_side, sz=sz,
                ...
                print(f"[Alpha] DB save error: {e}", flush=True)

        self._equity -= fee
        print(f"[Alpha] OPEN  {now[:19]} {coin:4} {side:5} "
    ...
    async def _check_and_trade(self):
        ...
        if not client:
            print("[Alpha] No OKX client available", flush=True)
            return
        ...
        for coin in cfg.symbols:
            try:
                ...
                print(f"[Alpha] Error fetching {coin}: {e}", flush=True)
        ...
        for coin, ind in indicators.items():
            ...
            if ind["avg_atr_30"] > 0:
                if ind["atr"] > ind["avg_atr_30"] * cfg.vol_mult:
                    print(f"[Alpha] Vol filter: {coin} ATR={ind['atr']:.1f} > "
            ...
            if ind["rsi"] > cfg.rsi_long_max and ind["ema_trend"]:
                print(f"[Alpha] RSI filter: {coin} RSI={ind['rsi']:.1f} > {cfg.rsi_long_max} -> no long", flush=True)
                continue
            if ind["rsi"] < cfg.rsi_short_min and not ind["ema_trend"]:
                print(f"[Alpha] RSI filter: {coin} RSI={ind['rsi']:.1f} < {cfg.rsi_short_min} -> no short", flush=True)
                continue
            ...
            if btc_trend_bull and not btc_above_200ma:
                ...
                if ind["roc"] > 0 and ind["ema_trend"]:
                    print(f"[Alpha] Bear filter: {coin} -> skip long (BTC < 200MA)", flush=True)
                    continue
    ...
    async def _poll_loop(self):
        ...
        while self._running:
            try:
                ...
                print(f"[Alpha] Poll error: {e}", flush=True)
    ...
    async def stop(self):
        ...
        if self.db:
            try:
                await self.db.update_bot_stopped(ALPHA_BOT_ID)
            except Exception:
                pass
    ...
    async def _sync_positions_db(self):
        ...
        try:
            if self.db._pg_mode:
                await self.db._execute("DELETE FROM positions WHERE bot_id = $1", (ALPHA_BOT_ID,))
            else:
                await self.db._execute("DELETE FROM positions WHERE bot_id = ?", (ALPHA_BOT_ID,))
            for coin, pos in self._positions.items():
                await self.db.save_position(
                    bot_id=ALPHA_BOT_ID, inst_id=pos.inst_id,
                    side=pos.side, size=pos.size,
                    entry_price=round(pos.entry_price, 2),
                    current_price=pos.peak_price,
                )
        except Exception as e:
            print(f"[Alpha] DB sync positions error: {e}", flush=True)
        ...
        try:
            ...
            if self.db._pg_mode:
                await self.db._execute(
                    ...
                    "VALUES ($1, 'alpha', 'alpha_strategy', 'MULTI', '1D', "
                    "$2, $3, 'running', 'demo', 'momentum', $4, 'Alpha Strategy') "
                    "ON CONFLICT (id) DO NOTHING",
                    (ALPHA_BOT_ID, self._equity, str(params), now),
                )
            else:
                await self.db._execute(
                    "INSERT OR IGNORE INTO bots (id, strategy_id, strategy_code, symbol, timeframe, "
                    "capital, params, status, mode, signal_type, created_at, name) "
                    "VALUES (?, 'alpha', 'alpha_strategy', 'MULTI', '1D', "
                    "?, ?, 'running', 'demo', 'momentum', ?, 'Alpha Strategy')",
                    (ALPHA_BOT_ID, self._equity, str(params), now),
                )
        except Exception as e:
            print(f"[Alpha] DB ensure_bot error: {e}", flush=True)

    async def _reload_equity(self):
        if not self.db:
            return
        try:
            rows = await self.db.get_trades(bot_id=ALPHA_BOT_ID, limit=500)
            for t in rows:
