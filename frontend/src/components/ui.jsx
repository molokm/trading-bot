...
/* ═══════ Glossary Modal ═══════ */
const GLOSSARY = [
  { term: 'PnL (Profit & Loss)', desc: 'Прибыль или убыток. Positive PnL = вы заработали, Negative PnL = вы потеряли.' },
  { term: 'ROI', desc: 'Возврат инвестиций. (Текущая стоимость − Начальные вложения) / Начальные вложения × 100%.' },
  { term: 'Win Rate', desc: 'Процент прибыльных сделок от общего числа. Win Rate 60% означает, что 60 из 100 сделок были прибыльными.' },
  { term: 'Profit Factor', desc: 'Отношение валовой прибыли к валовому убытку. PF > 1 — стратегия прибыльна. PF = 2 означает $2 прибыли на каждый $1 убытка.' },
  { term: 'Sharpe Ratio', desc: 'Коэффициент Шарпа — меряет доходность относительно риска. > 1 — хорошо, > 2 — отлично, < 0 — лучше не торговать.' },
  { term: 'Max Drawdown', desc: 'Максимальное падение капитала от пика. MDD 20% означает, что в худший момент вы потеряли 20% от максимума.' },
  { term: 'Take Profit (TP)', desc: 'Ордер на закрытие позиции с прибылью при достижении целевой цены.' },
  { term: 'Stop Loss (SL)', desc: 'Ордер на закрытие позиции с убытком при достижении цены стопа для ограничения потерь.' },
  { term: 'Trailing Stop', desc: 'Плавающий стоп, который следует за ценой. Фиксирует прибыль, двигая стоп-уровень вслед за растущей ценой.' },
  { term: 'Breakeven', desc: 'Безубыток — уровень цены, при котором позиция закрывается без прибыли и убытка (комиссии не учитываются).' },
  { term: 'Grid Strategy', desc: 'Сеточная стратегия — расстановка лимитных ордеров на равных интервалах. Хорошо работает в боковике (флэте).' },
  { term: 'DCA (Dollar Cost Averaging)', desc: 'Усреднение долларовой стоимости — покупка дополнительного объёма при падении цены для снижения средней цены входа.' },
  { term: 'Scalping', desc: 'Скальпинг — множество быстрых сделок с малым профитом. Используется на волатильных рынках.' },
  { term: 'ROE', desc: 'Return on Equity — доходность вложенных средств. ROE 50% означает, что на каждый $1 маржи вы заработали $0.50.' },
]
